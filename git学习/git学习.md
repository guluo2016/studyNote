...
*git fetch*
- 当远程仓库中有更新的时候（如别的用户提交了新内容，新建了新分支），这个时候自己的本地仓库是不知道的，只有本地获取到这些更新信息后，本地仓库才知道远程仓库上其他用户对其做了那些修改，使用的命令就是`git fetch [分支名]` ,如果没有写分支名，默认是获取远程仓库的所有分支的更新信息，git会把更新信息写入到.git文件夹下

*git checkout -b rpc origin/分支名*
- 本地仓库没有的分支，可以使用该命令从远程仓库同步一个新分支到本地